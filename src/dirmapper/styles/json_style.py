...
class JSONStyle(BaseStyle):
    def write_structure(self, structure: List[Tuple[str, int]]) -> dict:
        def build_json_structure(structure, level):
            result = {}
            i = 0
            while i < len(structure):
                item_path, item_level = structure[i]
                if item_level == level:
                    if os.path.isdir(item_path):
                        dir_name = os.path.basename(item_path)
                        sub_structure, sub_items = build_json_structure(structure[i+1:], level + 1)
                        result[dir_name] = sub_structure
                        i += sub_items
                    else:
                        result[os.path.basename(item_path)] = None
